...
const type3 = new Typed(".interface-text", {
    ...
    onStringTyped: function(arrayPos, self) {
        ...
        const interfaceTextElement = document.querySelector(".interface-text .typed-cursor"); // Get the element containing the typed text
        if (interfaceTextElement) {
            interfaceTextElement.previousSibling.innerHTML = interfaceTextElement.previousSibling.innerHTML.replace(/gh/g, '<span class="blue-text">gh</span>'); // Change color of "gh" to blue
            interfaceTextElement.previousSibling.innerHTML = interfaceTextElement.previousSibling.innerHTML.replace(/status/g, '<span class="yellow-text">status</span>'); // Change color of "status" to yellow
            interfaceTextElement.previousSibling.innerHTML = interfaceTextElement.previousSibling.innerHTML.replace(/\[#?(\d+)\]/g, '[<span class="green-text">$1</span>]'); // Change color of numbers within brackets to green
            interfaceTextElement.previousSibling.innerHTML = interfaceTextElement.previousSibling.innerHTML.replace(/https?:\/\/\S+/g, '<span class="green-text">$&</span>'); // Change color of links to green
        }
    }
});
